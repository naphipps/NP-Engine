...
namespace np
{
    namespace noise
    {
        ...
        private:
            ...
            ui8 _warp_octave_count; /// recommend [1, 2] - 0 gives you fbm - > 3 is not very usable due to performance but produces cool results - default to 1 for performance reasons
            flt _warp_octave_multiplier; /// similar to frequency, as this adds more and more "folds" to the warped shape - default to 1
            flt _warp_octave_increment; /// recommend [0, 1] => [smooth, clumpy] but can be whatever - this is a very small detail - 0.19 was chosen as default
            flt _warp_octave_displacement; /// recommend [0] for performance reasons - can be any value - only change if 0 is giving you too "regular" warping
            ...
            flt _fractional_increment; ///how much the fractional method increments to prevent fractal artifacts (default to 0.19)
            ui8 _octave_count; ///recommends [3, 6] for good control of detail - inputs [6, +oo) can be way much detail and perform slow - inputs [0, 3] may be too little detail - default to 3 for performance reasons
            ...
            inline static flt Grad(i32 hash, flt x, flt y, flt z, flt w)
            {
                ...
                return
                x * GRADIENT4D[i] + y * GRADIENT4D[i + 1] +
                z * GRADIENT4D[i + 2] + w * GRADIENT4D[i + 3];
            }
        ...
        public:
            ...
            /**
             constructor
             */
            Simplex(const random::Random32& random_engine = random::Random32()):
            random::Random32Base(random_engine)
            {
                Init();
            }
            ...
            /**
             ...
             recommend inputs [1, 2] - input  0 gives you fbm - inputs [3, +oo) is not very usable due to performance but produces cool results
             */
            ...
            /**
             ...
             recommend inputs [0, 1] => [smooth, clumpy] but can be whatever - this is a very small detail - 0.19 was chosen as default
             */
            ...
            /**
             ...
             recommend [0] for performance reasons - can be any value - recommend to change iff 0 is giving you too "regular" or "boring" warping and if so then recommend arbitrary input 0.19
             */
            ...
            /**
             fractal noise 2D - the same layer of noise will be added on top of itself an octave count number of times with detail respect to lacunarity
             Note: fractal noise may contain artifacts due to it layers the same layer of noise on top of itself
             output range [-1, 1]
             */
            ...
            /**
             fractal noise 3D - the same layer of noise will be added on top of itself an octave count number of times with detail respect to lacunarity
             Note: fractal noise may contain artifacts due to it layers the same layer of noise on top of itself
             output range [-1, 1]
             */
            ...
            /**
             fractal noise 4D - the same layer of noise will be added on top of itself an octave count number of times with detail respect to lacunarity
             Note: fractal noise may contain artifacts due to it layers the same layer of noise on top of itself
             output range [-1, 1]
             */
            ...
            /**
             fractional noise 1D - adds an octave count number of layers of noise together with detail respect to lacunarity - the added layers are fractional incremenet's value away from each other in 2D
             Note: if fractional increment is zero, then fractional noise may contain artifacts due to it layers the same layer of noise on top of itself
             output range [-1, 1]
             */
            ...
            /**
             fractional noise 2D - adds an octave count number of layers of noise together with detail respect to lacunarity - the added layers are fractional incremenet's value away from each other in 3D
             Note: if fractional increment is zero, then fractional noise may contain artifacts due to it layers the same layer of noise on top of itself
             output range [-1, 1]
             */
            ...
            /**
             fractional noise 3D - adds an octave count number of layers of noise together with detail respect to lacunarity - the added layers are fractional incremenet's value away from each other in 4D
             Note: if fractional increment is zero, then fractional noise may contain artifacts due to it layers the same layer of noise on top of itself
             output range [-1, 1]
             */
            ...
            /**
             rigid fractal noise 1D - provides the same noise as fractal 1D but adds the abs value of each layer then subtracts it from 1 then raises it to the rigidity value's power
             output range [0, 1]
             */
            ...
            /**
             rigid fractal noise 2D - provides the same noise as fractal 2D but adds the abs value of each layer then subtracts it from 1 then raises it to the rigidity value's power
             output range [0, 1]
             */
            ...
            /**
             rigid fractal noise 3D - provides the same noise as fractal 3D but adds the abs value of each layer then subtracts it from 1 then raises it to the rigidity value's power
             output range [0, 1]
             */
            ...
            /**
             rigid fractal noise 4D - provides the same noise as fractal 4D but adds the abs value of each layer then subtracts it from 1 then raises it to the rigidity value's power
             output range [0, 1]
             */
            ...
            /**
             rigid fractional noise 1D - provides the same noise as fractional 1D but adds the abs value of each layer then subtracts it from 1 then raises it to the rigidity value's power
             output range [0, 1]
             */
            ...
            /**
             rigid fractional noise 2D - provides the same noise as fractional 2D but adds the abs value of each layer then subtracts it from 1 then raises it to the rigidity value's power
             output range [0, 1]
             */
            ...
            /**
             rigid fractional noise 3D - provides the same noise as fractional 3D but adds the abs value of each layer then subtracts it from 1 then raises it to the rigidity value's power
             output range [0, 1]
             */
                ...
                else
                {
                    for (ui8 i=0; i<woc; i++)
                    {
                        ...
                        wy = Fractal(x + wom * wx + (i + wod) * woi,
                                     y + wom * wy + (i + wod) * woi,
                                     z + wom * wz + (i + wod) * woi);
                        wz = Fractal(x + wom * wx + (i + 2 * wod) * woi,
                                     y + wom * wy + (i + 2 * wod) * woi,
                ...
                else
                {
                    for (ui8 i=0; i<woc; i++)
                    {
                        ...
                        wz = Fractal(x + wom * wx + (i + 2 * wod) * woi,
                                     y + wom * wy + (i + 2 * wod) * woi,
                                     z + wom * wz + (i + 2 * wod) * woi,
                                     w + wom * ww + (i + 2 * wod) * woi);
                        ww = Fractal(x + wom * wx + (i + 4 * wod) * woi,
                                     y + wom * wy + (i + 4 * wod) * woi,
                                     z + wom * wz + (i + 4 * wod) * woi,
                                     w + wom * ww + (i + 4 * wod) * woi);
                    }
            ...
            /**
             warpped fractional 1D - provides the same noise as fractional 1D but warps it through itself based on warp related fields
             */
            ...
            /**
             warpped fractional 2D - provides the same noise as fractional 2D but warps it through itself based on warp related fields
             */
            inline flt WarpFractional(flt x, flt y) const
            {
                ...
                else
                {
                    for (ui8 i=0; i<woc; i++)
                    {
                        wx = Fractional(x + wom * wx + i * woi,
                                        y + wom * wy + i * woi);
                        wy = Fractional(x + wom * wx + (i + wod) * woi,
                                        y + wom * wy + (i + wod) * woi);
                    }
            ...
            /**
             warpped fractional 3D - provides the same noise as fractional 3D but warps it through itself based on warp related fields
             */
                ...
                else
                {
                    for (ui8 i=0; i<woc; i++)
                    {
                        ...
                        wy = Fractional(x + wom * wx + (i + wod) * woi,
                                        y + wom * wy + (i + wod) * woi,
                                        z + wom * wz + (i + wod) * woi);
                        wz = Fractional(x + wom * wx + (i + 2 * wod) * woi,
                                        y + wom * wy + (i + 2 * wod) * woi,
                                        z + wom * wz + (i + 2 * wod) * woi);
                    }
