    ...
    namespace noise
    {
        ...
        public:
            ...
            /**
             constructor
             */
            Perlin(const random::Random32& engine = random::Random32()):
            random::Random32Base(engine)
            {
                Init();
            }
            ...
            inline flt CalculateNoise(flt x, flt y, flt z) const
            {
                ...
                flt res = math::lerp(math::lerp(math::lerp(Grad(_permutation[AA], x, y, z),
                                                           Grad(_permutation[BA], x-1, y, z),
                                                           u),
                                                math::lerp(Grad(_permutation[AB], x, y-1, z),
                                                           Grad(_permutation[BB], x-1, y-1, z),
                                                           u),
                                                v),
                                     math::lerp(math::lerp(Grad(_permutation[AA+1], x, y, z-1),
                                                           Grad(_permutation[BA+1], x-1, y, z-1),
                                                           u),
                                                math::lerp(Grad(_permutation[AB+1], x, y-1, z-1),
            ...
            /**
             ...
             Note: if fractional increment is zerp, then fractional noise may contain artifacts due to it layers the same layer of noise on top of itself
             output range [-1, 1]
             */
            inline flt Fractional(flt x, flt y) const
            {
                ...
                for (ui8 i = 0; i < _octave_count; i++)
                {
                    output += amplitude * CalculateNoise(x * frequency, y * frequency,
                                                         i * fractionalIncrement * frequency);
                    denom += amplitude;
                    frequency *= _lacunarity;
                    amplitude *= _persistence;
                }
