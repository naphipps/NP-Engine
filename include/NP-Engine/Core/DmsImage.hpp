...
namespace np
{
	...
	{
		/**
		 DmsImage is to be treated as a 2D array of ui16 elements and provides shape extraction via the dual marching squares
		 algorithm DmsImage inner array is created on construction
		 */
		...
		private:
			 ...
			 */
			inline void GetLineSegmentsAroundPoint(DmsLineSegmentUset& segments, const math::ui16Point point,
												   const flt isohreshold)
			{
				...
				if (point.x == 0 || point.x == base::GetWidth() - 1 || point.y == 0 || point.y == base::GetHeight() - 1)
				{
			...
			inline flt GetBorderIsovalue(const math::fltPoint point)
			{
				...
				if (point.x >= 0 && point.x < base::GetWidth())
				{
					if (point.y >= 0 && point.y < base::GetHeight())
					{
						...
						math::ui16Point point_as_ui16{.x = (ui16)floor(point.x), .y = (ui16)floor(point.y)};
			...
			/**
			 gets the line segments given iso threshold around given point along the outside border of this image and stores
			 them into give line segment uset
			 */
			inline void GetBorderLineSegmentsAroundPoint(DmsLineSegmentUset& segments, const math::ui16Point point,
														 const flt isohreshold)
			{
				/* point names in reference of given point
				 upper_left     upper       upper_right
				...
				math::fltPoint fPoint{.x = (flt)point.x, .y = (flt)point.y};
			...
			/**
			 gets the line segments given iso threshold around given point along the outside border of this image and stores
			 them into give line segment uset this method is meant to be used by GetBorderLineSegmentsAroundPoint
			 */
			inline void GetBorderLineSegment(DmsLineSegmentUset& segments, const math::ui16Point point, const flt isohreshold,
											 const math::fltPoint llPoint, const math::fltPoint urPoint)
			{
				...
				case 2:
					...
					break;
				case 3:
					if (fPoint == lrPoint || fPoint == urPoint)
					{
						// return;
				...
				case 5:
					...
					break;
				case 6:
					if (fPoint == ulPoint || fPoint == urPoint)
					{
						// return;
					...
					break;
				case 7:
					if (fPoint == ulPoint || fPoint == urPoint || fPoint == lrPoint)
					{
						return;
				...
				case 8:
					...
					break;
				case 9:
					if (fPoint == llPoint || fPoint == lrPoint)
					{
						// return;
				...
				case 10:
					...
					break;
				case 11:
					if (fPoint == llPoint || fPoint == urPoint || fPoint == lrPoint)
					{
						return;
					...
					break;
				case 12:
					if (fPoint == llPoint || fPoint == ulPoint)
					{
						// return;
					...
					break;
				case 13:
					if (fPoint == llPoint || fPoint == ulPoint || fPoint == lrPoint)
					{
						return;
					...
					break;
				case 14:
					if (fPoint == llPoint || fPoint == ulPoint || fPoint == urPoint)
					{
						return;
				...
				case 15:
				{
					...
					if (corner_begin != corner_end)
					{
						if (left_edge)
						{
							segments.insert(DmsLineSegment(corner_begin, corner_end, point, corner_begin));
						}
						else // right_edge
						{
							segments.insert(DmsLineSegment(corner_begin, corner_end, point, corner_end));
						}
					}
			 ...
			 */
			inline void GetLineSegment(DmsLineSegmentUset& segments, const math::ui16Point point, const flt isohreshold,
									   const math::ui16Point llPoint, const math::ui16Point urPoint)
			{
				...
				case 2:
					...
					break;
				case 3:
					if (point == lrPoint || point == urPoint)
					{
						flt begin_t = (isohreshold - ll_iso) / (lr_iso - ll_iso);
						flt end_t = (isohreshold - ul_iso) / (ur_iso - ul_iso);
				...
				case 5:
					...
					break;
				case 6:
					if (point == ulPoint || point == urPoint)
					{
						flt begin_t = (isohreshold - ll_iso) / (ul_iso - ll_iso);
						flt end_t = (isohreshold - lr_iso) / (ur_iso - lr_iso);
					...
					break;
				case 7:
					if (point == ulPoint || point == urPoint || point == lrPoint)
					{
						flt begin_t = (isohreshold - ll_iso) / (ul_iso - ll_iso);
						flt end_t = (isohreshold - ll_iso) / (lr_iso - ll_iso);
				...
				case 8:
					...
					break;
				case 9:
					if (point == llPoint || point == lrPoint)
					{
						flt begin_t = (isohreshold - ul_iso) / (ll_iso - ul_iso);
						flt end_t = (isohreshold - ur_iso) / (lr_iso - ur_iso);
				...
				case 10:
					...
					break;
				case 11:
					if (point == llPoint || point == urPoint || point == lrPoint)
					{
						flt begin_t = (isohreshold - ul_iso) / (ll_iso - ul_iso);
						flt end_t = (isohreshold - ul_iso) / (ur_iso - ul_iso);
					...
					break;
				case 12:
					if (point == llPoint || point == ulPoint)
					{
						flt begin_t = (isohreshold - lr_iso) / (ll_iso - lr_iso);
						flt end_t = (isohreshold - ur_iso) / (ul_iso - ur_iso);
					...
					break;
				case 13:
					if (point == llPoint || point == ulPoint || point == lrPoint)
					{
						flt begin_t = (isohreshold - ur_iso) / (ul_iso - ur_iso);
						flt end_t = (isohreshold - ur_iso) / (lr_iso - ur_iso);
					...
					break;
				case 14:
					if (point == llPoint || point == ulPoint || point == urPoint)
					{
						flt begin_t = (isohreshold - lr_iso) / (ll_iso - lr_iso);
						flt end_t = (isohreshold - lr_iso) / (ur_iso - lr_iso);
		...
		public:
			...
			/**
			 constructor passing to ui16_drawable_image
			 */
			DmsImage(const ui16 value): base(value) {}

			/**
			 constructor passing to ui16_drawable_image
			 */
			DmsImage(const ui16 value, const math::ui16Point origin): base(value, origin) {}

			/**
			 constructor passing to ui16_drawable_image
			 */
			DmsImage(const math::ui16Point origin): base(origin) {}
			...
			/**
			 gets the dms iso value [0.0 - 1.0] from given point - isovalue  => fraction of maximum value => 128 / 255 =
			 0.501960784313725
			 */
			...
			/**
			 gets the dms iso value [0.0 - 1.0] from given (x, y) - isovalue  => fraction of maximum value => 128 / 255 =
			 0.501960784313725
			 */
			inline virtual flt GetIsovalue(const ui16 x, const ui16 y) const
			{
				// TODO: make GetIsovalue more flexible (defaulting to 0 or 1?) -- make a CalculateIsovalue(value) function
				// pointer?
					...
					for (ui16 x = 0; x < base::GetWidth(); x++)
					{
							...
							if (GetIsovalue(front) >= isohreshold)
							{
								...
								if (!visited.GetValue(front))
								{
									...
									if (front.y + 1 < base::GetHeight())
									{
										...
										{
											floodFillQueue.push(
												math::ui16Point{.x = front.x, .y = static_cast<ui16>(front.y + 1)});
										}
									}
									if (front.y - 1 >= 0)
									{
									...
									if (front.x + 1 < base::GetWidth())
									{
										...
										{
											floodFillQueue.push(
												math::ui16Point{.x = static_cast<ui16>(front.x + 1), .y = front.y});
										}
									}
						...
						if (segments->size() > 0)
						{
							...
							while (enable && !segments->empty())
							{
								...
								for (DmsLineSegment seg : *segments)
								{
									if (seg.Begin() == loop_iterator->back().Begin() ||
										...
										seg.End() == loop_iterator->back().Begin() || seg.End() == loop_iterator->back().End())
									{
							...
							::std::sort(line_loops.begin(), line_loops.end(),
										[](const DmsLineSegmentVector& a, const DmsLineSegmentVector& b) -> bl
										{
