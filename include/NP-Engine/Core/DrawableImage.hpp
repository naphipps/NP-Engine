	...
	namespace core
	{
		...
		public:
			...
			DrawableImage():
				...
				_aabb(math::ui16AABB{.LowerLeft = _origin,
									 .UpperRight = math::ui16Point{.x = base::GetWidth(), .y = base::GetHeight()}})
			{}
			...
			DrawableImage(const ui16 value, const math::ui16Point origin):
				...
				_origin(origin),
				_aabb(math::ui16AABB{.LowerLeft = _origin,
									 .UpperRight = math::ui16Point{.x = static_cast<ui16>(_origin.x + base::GetWidth()),
			 ...
			 */
			inline math::fltPointVector FloodFillWithNeighbors(const math::ui16Point point, const T& oldValue,
															   const T& newValue)
			{
				...
				while (!queue.empty())
				{
					...
					if (GetValue(front) == oldValue)
					{
						...
						{
							neighbors.push_back(math::fltPoint{.x = (flt)front.x, .y = static_cast<flt>((flt)front.y + 1.f)});
						}
						...
						{
							neighbors.push_back(math::fltPoint{.x = static_cast<flt>((flt)front.x + 1.f), .y = (flt)front.y});
						}
