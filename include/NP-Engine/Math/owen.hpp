...
namespace np::mat
{
	namespace __detail
	{
		...
		*/
		static inline dbl tfun(dbl h, dbl a, dbl ah)
		{
			dbl ai;
			dbl aj;
			dbl arange[7] = {0.025, 0.09, 0.15,	  0.36,
							 0.5,	0.9,  0.99999

			};
			dbl as;
			dbl c2[21] = {0.99999999999999987510,	  -0.99999999999988796462,	   0.99999999998290743652,
						  -0.99999999896282500134,	  0.99999996660459362918,	   -0.99999933986272476760,
						  ...
						  0.89246305511006708555,	  -0.76893425990463999675,	   0.58893528468484693250,
						  -0.38380345160440256652,	  0.20317601701045299653,	   -0.82813631607004984866E-01,
						  0.24167984735759576523E-01, -0.44676566663971825242E-02, 0.39141169402373836468E-03};
			dbl dhs;
			dbl dj;
			dbl gj;
			dbl hrange[14] = {0.02, 0.06, 0.09, 0.125, 0.26, 0.4, 0.6, 1.6, 1.7, 2.33, 2.4, 3.36, 3.4, 4.8};
			dbl hs;
			i32 i;
			i32 iaint;
			i32 icode;
			...
			i32 m;
			i32 maxii;
			i32 meth[18] = {1, 1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 3, 4, 4, 4, 4, 5, 6};
			dbl normh;
			i32 ord[18] = {2, 3, 4, 5, 7, 10, 12, 18, 10, 20, 30, 20, 4, 7, 8, 20, 13, 0};
			dbl pts[13] = {0.35082039676451715489E-02, 0.31279042338030753740E-01, 0.85266826283219451090E-01,
						   ...
						   0.81475510988760098605,	   0.89711029755948965867,	   0.95723808085944261843,
						   0.99178832974629703586};
			dbl r;
			dbl rrtpi = 0.39894228040143267794;
			dbl rtwopi = 0.15915494309189533577;
			i32 select[15 * 8] = {
				1, 1, 2, 13, 13, 13, 13, 13, 13, 13, 13, 16, 16, 16, 9,	 1, 2, 2, 3, 3, 5, 5,  14, 14, 15, 15, 16, 16, 16, 9,
			...
			dbl value;
			dbl vi;
			dbl wts[13] = {0.18831438115323502887E-01, 0.18567086243977649478E-01, 0.18042093461223385584E-01,
			...
			//  jj = 2j - 1 ; gj = ::std::exp(-h*h/2) * (-h*h/2)**j / j//
			//  aj = a**(2j-1) / (2*pi)
			//
			if (meth[icode - 1] == 1)
			{
			...
			//  z = (-1)**(i-1) * zi ; ii = 2i - 1
			//  vi = (-1)**(i-1) * a**(2i-1) * ::std::exp[-(a*h)**2/2] / ::std::sqrt(2*pi)
			//
			else if (meth[icode - 1] == 2)
			{
				...
				for (;;)
				{
					...
					if (maxii <= ii)
					{
						value = value * rrtpi * ::std::exp(-0.5 * hs);
						return value;
					}
					z = y * (vi - (dbl)(ii)*z);
					vi = as * vi;
			...
			//  t3(h, a, m) ; m = 20
			//  ii = 2i - 1
			//  vi = a**(2i-1) * ::std::exp[-(a*h)**2/2] / ::std::sqrt(2*pi)
			//
			...
			//  ai = a * ::std::exp[-h*h*(1+a*a)/2] * (-a*a)**i / (2*pi)
			//
			else if (meth[icode - 1] == 4)
			{
				...
				as = -a * a;
				value = 0.0;
				ai = rtwopi * a * ::std::exp(-0.5 * hs * (1.0 - as));
				yi = 1.0;
			...
			else if (meth[icode - 1] == 5)
			{
				...
				{
					r = 1.0 + as * pts[i - 1];
					value = value + wts[i - 1] * ::std::exp(hs * r) / r;
				}
				value = a * value;
			}
			//
	...
	*/
	static inline dbl owen_t(dbl h, dbl a)
	{
		...
		{
			value = 0.25 - __detail::znorm1(absh) * __detail::znorm1(ah) - __detail::tfun(ah, 1.0 / absa, absh);
		}
		else
		{
			normh = __detail::znorm2(absh);
			normah = __detail::znorm2(ah);
			value = 0.5 * (normh + normah) - normh * normah - __detail::tfun(ah, 1.0 / absa, absh);
		}
