...
namespace np
{
	namespace math
	{
		 ...
		 */
		static inline bl GetMidpointCirclePoints(const flt radius, ui16Point* points, ui32& points_size,
												 const ui32 points_max_size, const ui16Point& offset)
		{
			ui32 i = 0;
			...
			if (result)
			{
				::std::sort(points, points + points_size,
							[&offset](const ui16Point& a, const ui16Point& b)
							{
		...
		static inline bl GetMidpointCirclePoints(const flt radius, ui16PointArray<ARRAY_SIZE>& points, ui32& points_size,
												 const ui16Point& offset)
		{
			ui32 i = 0;
			...
			if (result)
			{
				::std::sort(points.begin(), points.begin() + points_size,
							[&offset](const ui16Point& a, const ui16Point& b)
							{
								return atan2(-((flt)a.y) + (flt)offset.y, -((flt)a.x) + (flt)offset.x) <
									atan2(-((flt)b.y) + (flt)offset.y, -((flt)b.x) + (flt)offset.x);
