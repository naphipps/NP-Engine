...
namespace np
{
	namespace math
	{
		...
		{
			return fltPoint{.x = lerp((flt)a.x, (flt)b.x, t), .y = lerp((flt)a.y, (flt)b.y, t)};
		}
		...
		{
			return fltPoint{.x = ((flt)a.x + (flt)b.x) / 2.0f, .y = ((flt)a.y + (flt)b.y) / 2.0f};
		}
		...
		static inline dbl getOrientation(const fltPoint& a, const fltPoint& b, const fltPoint& c)
		{
			...
			dbl result =
				(bx * cy) * 0.5 - (by * cx) * 0.5 - (ax * cy) * 0.5 + (ay * cx) * 0.5 + (ax * by) * 0.5 - (ay * bx) * 0.5;
