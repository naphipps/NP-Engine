...
namespace np::mem
{
	...
	public:
		using ObjectType = typename PoolType::ObjectType; //TODO: do we need "typename"??
		...
		ObjectTypePtr CreateObject(Args&&... args)
		{
			...
			for (siz i = 0; i < _pools.size() && !object; i++) 
				object = _pools[i]->CreateObject<Args>(::std::forward<Args>(args)...);

			if (!object)
			{
				siz size = CalcAlignedSize(sizeof(PoolType)) + (A::CHUNK_SIZE * NP_ENGINE_ACCUMULATING_POOL_OBJECT_COUNT);
				...
				object = _pools.back()->CreateObject<Args>(::std::forward<Args>(args)...);
			}
