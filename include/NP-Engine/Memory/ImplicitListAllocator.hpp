			...
			bl Deallocate(void* ptr) override
			{
				...
				if (Contains(ptr))
				{
						 ...
						 prev_footer = (ui8*)header - __detail::MARGIN_ALIGNED_SIZE)
					{
						...
						{
							header =
								(MarginPtr)(prev_footer + __detail::MARGIN_ALIGNED_SIZE - ((MarginPtr)prev_footer)->GetSize());
						}
					}
						 ...
						 next_header = (ui8*)footer + __detail::MARGIN_ALIGNED_SIZE)
					{
						...
						{
							footer =
								(MarginPtr)(next_header - __detail::MARGIN_ALIGNED_SIZE + ((MarginPtr)next_header)->GetSize());
						}
					}
