...
#define NP_PLATFORM_IS_APPLE true
#else
#define NP_PLATFORM_IS_APPLE false
#endif

#if defined(__WINDOWS__) || defined(_WIN32) || defined(_WIN64) || defined(__WIN32__) || defined(__TOS_WIN__)
#define NP_PLATFORM_IS_WINDOWS true
#else
#define NP_PLATFORM_IS_WINDOWS false
#endif

#if defined(__linux__) || defined(__linux) || defined(linux)
#define NP_PLATFORM_IS_LINUX true
#else
#define NP_PLATFORM_IS_LINUX false
#endif

//ensure only one platform is detected
#if NP_PLATFORM_IS_APPLE && NP_PLATFORM_IS_WINDOWS
#error NP detected Apple and Windows - only one is allowed
#elif NP_PLATFORM_IS_APPLE && NP_PLATFORM_IS_LINUX
#error NP detected Apple and Linux - only one is allowed
#elif NP_PLATFORM_IS_WINDOWS && NP_PLATFORM_IS_LINUX
#error NP detected Windows and Linux - only one is allowed
#endif

//perform platform checks
#if NP_PLATFORM_IS_WINDOWS && defined(_WIN32) && !defined(_WIN64)
#error NP detected x86 platform - NP does not support

#elif NP_PLATFORM_IS_APPLE
