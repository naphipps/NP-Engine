...
#include "PolygonUtility.hpp"

//---------------------------------------------------------------------------
// PolygonUtility
//---------------------------------------------------------------------------

np::math::i32PointVector PolygonUtility::Simplify(const np::math::i32PointVector& polygon)
{
	np::math::i32PointVector simplifiedPolygon(polygon);
	float tolerance = 0.f;

	if (simplifiedPolygon.size() > 3)
	{
		float infinity = std::numeric_limits<float>::infinity();

		// refine the original list
		for (int i = 0; i < simplifiedPolygon.size() - 2;)
		{
			float firstSlope = simplifiedPolygon[i + 1].y - simplifiedPolygon[i].y;
			float run = simplifiedPolygon[i + 1].x - simplifiedPolygon[i].x;
			firstSlope = run == 0 ? infinity : firstSlope / run;

			float secondSlope = simplifiedPolygon[i + 2].y - simplifiedPolygon[i + 1].y;
			run = simplifiedPolygon[i + 2].x - simplifiedPolygon[i + 1].x;
			secondSlope = run == 0 ? infinity : secondSlope / run;

			if (abs(secondSlope - firstSlope) <= tolerance || (firstSlope == infinity && secondSlope == infinity))
			{
				simplifiedPolygon.erase(simplifiedPolygon.begin() + i + 1);
			}
			else
			{
				i++;
		...
		float firstSlope = simplifiedPolygon[1].y - simplifiedPolygon.front().y;
		float run = simplifiedPolygon[1].x - simplifiedPolygon.front().x;
		firstSlope = run == 0 ? infinity : firstSlope / run;

		float lastSlope = simplifiedPolygon.front().y - simplifiedPolygon.back().y;
		run = simplifiedPolygon.front().x - simplifiedPolygon.back().x;
		lastSlope = run == 0 ? infinity : lastSlope / run;

		while (abs(firstSlope - lastSlope) <= tolerance || (firstSlope == infinity && lastSlope == infinity))
		{
			simplifiedPolygon.erase(simplifiedPolygon.begin());
			np::math::i32Point lastPoint = simplifiedPolygon.back();
			simplifiedPolygon.erase(simplifiedPolygon.end() - 1);
			simplifiedPolygon.insert(simplifiedPolygon.begin(), lastPoint);

			firstSlope = simplifiedPolygon[1].y - simplifiedPolygon.front().y;
			run = simplifiedPolygon[1].x - simplifiedPolygon.front().x;
			firstSlope = run == 0 ? infinity : firstSlope / run;

			lastSlope = simplifiedPolygon.front().y - simplifiedPolygon.back().y;
			run = simplifiedPolygon.front().x - simplifiedPolygon.back().x;
			lastSlope = run == 0 ? infinity : lastSlope / run;
		}
	}
...
np::math::fltPointVector PolygonUtility::Simplify(const np::math::fltPointVector& polygon)
{
	np::math::fltPointVector simplifiedPolygon(polygon);
	float tolerance = 0.f;
	...
	if (simplifiedPolygon.size() > 3)
	{
		...
		{
			simplifiedPolygon.erase(simplifiedPolygon.begin());
			np::math::fltPoint lastPoint = simplifiedPolygon.back();
			simplifiedPolygon.erase(simplifiedPolygon.end() - 1);
			simplifiedPolygon.insert(simplifiedPolygon.begin(), lastPoint);
...
float PolygonUtility::PolygonPartitioner::Distance(const np::math::fltPoint& p1, const np::math::fltPoint& p2)
{
	float dx, dy;
	dx = p2.x - p1.x;
	dy = p2.y - p1.y;
...
int PolygonUtility::PolygonPartitioner::Intersects(np::math::fltPoint& p11, np::math::fltPoint& p12, np::math::fltPoint& p21,
												   np::math::fltPoint& p22)
{
	...
	np::math::fltPoint v1ort, v2ort, v;
...
int PolygonUtility::PolygonPartitioner::RemoveHoles(PolygonVector* inpolys, PolygonVector* outpolys)
{
	PolygonVector polys;
	PolygonVector::iterator holeiter, polyiter, iter, iter2;
	long i, i2, holepointindex, polypointindex;
	np::math::fltPoint holepoint, polypoint, bestpolypoint;
	np::math::fltPoint linep1, linep2;
	np::math::fltPoint v1, v2;
	Polygon newpoly;
	bool hasholes;
	bool pointvisible;
	bool pointfound;
	...
	while (1)
	{
		...
		for (iter = polys.begin(); iter != polys.end(); iter++)
		{
			...
			for (i = 0; i < iter->GetNumPoints(); i++)
			{
				...
				if (pointfound)
				{
					np::math::fltPoint norm;
					norm.x = polypoint.x - holepoint.x;
					norm.y = polypoint.y - holepoint.y;
					//                    v1 = Normalize(polypoint-holepoint);
					v1 = Normalize(norm);

					norm.x = bestpolypoint.x - holepoint.x;
					norm.y = bestpolypoint.y - holepoint.y;
...
bool PolygonUtility::PolygonPartitioner::IsConvex(np::math::fltPoint& p1, np::math::fltPoint& p2, np::math::fltPoint& p3)
{
	float tmp;
...
bool PolygonUtility::PolygonPartitioner::IsReflex(np::math::fltPoint& p1, np::math::fltPoint& p2, np::math::fltPoint& p3)
{
	float tmp;
	tmp = (p3.y - p1.y) * (p2.x - p1.x) - (p3.x - p1.x) * (p2.y - p1.y);
	if (tmp < 0)
...
bool PolygonUtility::PolygonPartitioner::IsInside(np::math::fltPoint& p1, np::math::fltPoint& p2, np::math::fltPoint& p3,
												  np::math::fltPoint& p)
{
	if (IsConvex(p1, p, p2))
		return false;
...
bool PolygonUtility::PolygonPartitioner::InCone(np::math::fltPoint& p1, np::math::fltPoint& p2, np::math::fltPoint& p3,
												np::math::fltPoint& p)
{
	bool convex;
...
bool PolygonUtility::PolygonPartitioner::InCone(PartitionVertex* v, np::math::fltPoint& p)
{
	np::math::fltPoint p1, p2, p3;
...
int PolygonUtility::PolygonPartitioner::Triangulate_EC(Polygon* poly, PolygonVector* triangles)
{
	if (!poly->Valid())
		return 0;
...
int PolygonUtility::PolygonPartitioner::Triangulate_EC(PolygonVector* inpolys, PolygonVector* triangles)
{
	PolygonVector outpolys;
...
int PolygonUtility::PolygonPartitioner::ConvexPartition_HM(Polygon* poly, PolygonVector* parts)
{
	if (!poly->Valid())
		return 0;
...
int PolygonUtility::PolygonPartitioner::ConvexPartition_HM(PolygonVector* inpolys, PolygonVector* parts)
{
	PolygonVector outpolys;
	PolygonVector::iterator iter;
...
int PolygonUtility::PolygonPartitioner::Triangulate_OPT(Polygon* poly, PolygonVector* triangles)
{
	if (!poly->Valid())
		return 0;
	...
	while (!diagonals.empty())
	{
		...
		//        diagonals.pop_front();
		diagonals.erase(diagonals.begin());
...
void PolygonUtility::PolygonPartitioner::UpdateState(long a, long b, long w, long i, long j, DPState2** dpstates)
{
	...
	{
		pairs->clear();
		pairs->insert(pairs->begin(), newdiagonal);
...
int PolygonUtility::PolygonPartitioner::ConvexPartition_OPT(Polygon* poly, PolygonVector* parts)
{
	if (!poly->Valid())
		return 0;
	...
	newdiagonal.index1 = 0;
	newdiagonal.index2 = n - 1;
	diagonals.insert(diagonals.begin(), newdiagonal);
	...
	while (!diagonals.empty())
	{
		...
		diagonals2.insert(diagonals2.begin(), diagonal);
		//        diagonals2.push_front(diagonal);

		while (!diagonals2.empty())
		{
			diagonal = *(diagonals2.begin());
			diagonals2.erase(diagonals2.begin());
...
void PolygonUtility::PolygonPartitioner::AddDiagonal(MonotoneVertex* vertices, long* numvertices, long index1, long index2,
...
bool PolygonUtility::PolygonPartitioner::Below(np::math::fltPoint& p1, np::math::fltPoint& p2)
{
	if (p1.y < p2.y)
		return true;
	else if (p1.y == p2.y)
...
bool PolygonUtility::PolygonPartitioner::VertexSorter::operator()(long index1, long index2)
{
...
bool PolygonUtility::PolygonPartitioner::ScanLineEdge::IsConvex(const np::math::fltPoint& p1, const np::math::fltPoint& p2,
																const np::math::fltPoint& p3) const
{
	float tmp;
	tmp = (p3.y - p1.y) * (p2.x - p1.x) - (p3.x - p1.x) * (p2.y - p1.y);
	if (tmp > 0)
...
bool PolygonUtility::PolygonPartitioner::ScanLineEdge::operator<(const ScanLineEdge& other) const
{
...
int PolygonUtility::PolygonPartitioner::Triangulate_MONO(PolygonVector* inpolys, PolygonVector* triangles)
{
	PolygonVector monotone;
	PolygonVector::iterator iter;
...
int PolygonUtility::PolygonPartitioner::Triangulate_MONO(Polygon* poly, PolygonVector* triangles)
{
	PolygonVector polys;
...
int PolygonUtility::PolygonPartitioner::MonotonePartition(PolygonVector* inpolys, PolygonVector* monotonePolys)
{
	...
	for (i = 0; i < numvertices; i++)
	{
		...
		if (Below(vprev->p, v->p) && Below(vnext->p, v->p))
		{
			...
			{
				vertextypes[i] = POLYGON_UTILITY_VERTEXTYPE_START;
			}
			else
			{
				vertextypes[i] = POLYGON_UTILITY_VERTEXTYPE_SPLIT;
			}
		}
		else if (Below(v->p, vprev->p) && Below(v->p, vnext->p))
		{
			if (IsConvex(vnext->p, vprev->p, v->p))
			{
				vertextypes[i] = POLYGON_UTILITY_VERTEXTYPE_END;
			}
			else
			{
				vertextypes[i] = POLYGON_UTILITY_VERTEXTYPE_MERGE;
			}
		}
		else
		{
			vertextypes[i] = POLYGON_UTILITY_VERTEXTYPE_REGULAR;
		}
	}
	...
	for (i = 0; i < numvertices; i++)
	{
		...
		{
		case POLYGON_UTILITY_VERTEXTYPE_START:
		...
		case POLYGON_UTILITY_VERTEXTYPE_END:
			...
			if (vertextypes[helpers[v->previous]] == POLYGON_UTILITY_VERTEXTYPE_MERGE)
			{
				// Insert the diagonal connecting vi to helper(ei-1) in D.
				AddDiagonal(vertices, &newnumvertices, vindex, helpers[v->previous], vertextypes, edgeTreeIterators, &edgeTree,
							helpers);
			}
			// Delete ei-1 from T
		...
		case POLYGON_UTILITY_VERTEXTYPE_SPLIT:
			...
			AddDiagonal(vertices, &newnumvertices, vindex, helpers[edgeIter->index], vertextypes, edgeTreeIterators, &edgeTree,
						helpers);
		...
		case POLYGON_UTILITY_VERTEXTYPE_MERGE:
			if (edgeTreeIterators[v->previous] == edgeTree.end())
			{
				error = true;
				break;
			...
			if (vertextypes[helpers[edgeIter->index]] == POLYGON_UTILITY_VERTEXTYPE_MERGE)
			{
				// Insert the diagonal connecting vi to helper(e j) in D.
				AddDiagonal(vertices, &newnumvertices, vindex2, helpers[edgeIter->index], vertextypes, edgeTreeIterators,
							&edgeTree, helpers);
			}
			// helper(e j)�vi
		...
		case POLYGON_UTILITY_VERTEXTYPE_REGULAR:
			...
			else
			{
				...
				{
					// Insert the diagonal connecting vi to helper(e j) in D.
					AddDiagonal(vertices, &newnumvertices, vindex, helpers[edgeIter->index], vertextypes, edgeTreeIterators,
								&edgeTree, helpers);
				}
				// helper(e j)�vi
				helpers[edgeIter->index] = vindex;
...
int PolygonUtility::PolygonPartitioner::TriangulateMonotone(Polygon* inPoly, PolygonVector* triangles)
{
	if (!inPoly->Valid())
		return 0;
