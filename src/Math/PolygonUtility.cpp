...
int PolygonUtility::PolygonPartitioner::Intersects(np::math::fltPoint &p11, np::math::fltPoint &p12, np::math::fltPoint &p21, np::math::fltPoint &p22) {
...
bool PolygonUtility::PolygonPartitioner::IsInside(np::math::fltPoint& p1, np::math::fltPoint& p2, np::math::fltPoint& p3, np::math::fltPoint &p) {
...
bool PolygonUtility::PolygonPartitioner::InCone(np::math::fltPoint &p1, np::math::fltPoint &p2, np::math::fltPoint &p3, np::math::fltPoint &p) {
    bool convex;
...
//        pairs->push_front(newdiagonal);
        ...
        while((!pairs->empty())&&(pairs->begin()->index2 >= j)) pairs->erase(pairs->begin());
            ...
            if((diagonal.index2 - j) > 1) {
                if(iter->index1 != iter->index2) {
                    ...
                    while(1) {
                        ...
                        iter2 = pairs2->begin();
                        if(iter->index2 != iter2->index2) pairs2->erase(pairs2->begin());
...
bool PolygonUtility::PolygonPartitioner::ScanLineEdge::IsConvex(const np::math::fltPoint& p1, const np::math::fltPoint& p2, const np::math::fltPoint& p3) const {
    float tmp;
    tmp = (p3.y-p1.y)*(p2.x-p1.x)-(p3.x-p1.x)*(p2.y-p1.y);
    if(tmp>0) return 1;
    else return 0;
