...
namespace np::gpu
{
	mem::sptr<SamplerResource> SamplerResource::Create(mem::sptr<Device> device, SamplerResourceUsage usage, dbl anisotrophy, CompareOperation op, LodBounds lod_bounds,
		SamplerBorder border, SamplerAddressModes address_modes)
	{
		...
		case DetailType::Vulkan:
			resource = mem::create_sptr<__detail::VulkanSamplerResource>(device->GetServices()->GetAllocator(), device, usage, anisotrophy, op, lod_bounds, border, address_modes);
			break;
		}
