...
namespace np::gpu
{
	mem::sptr<RenderPass> RenderPass::Create(mem::sptr<Device> device, const con::vector<ImageResourceDescription>& descriptions,
		const con::vector<SubpassDescription>& subpasses, const con::vector<SubpassDependency>& dependencies)
	{
		...
		case DetailType::Vulkan:
			pass = mem::create_sptr<__detail::VulkanRenderPass>(device->GetServices()->GetAllocator(), device, descriptions, subpasses, dependencies);
			break;
