...
namespace np::gpu
{
	mem::sptr<ImageResourceView> ImageResourceView::Create(mem::sptr<Device> device, mem::sptr<ImageResource> image,
														   ImageResourceUsage usage)
	{
		...
		case DetailType::Vulkan:
			view = mem::create_sptr<__detail::VulkanImageResourceView>(device->GetServices()->GetAllocator(), device, image,
																	   usage);
			break;
		}
