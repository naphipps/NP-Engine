...
namespace np::gpu
{
	mem::sptr<GraphicsPipeline> GraphicsPipeline::Create(mem::sptr<RenderPass> render_pass,
		PipelineUsage usage,
		...
		mem::sptr<PipelineCache> cache)
	{
		...
		case DetailType::Vulkan:
			pipeline = mem::create_sptr<__detail::VulkanGraphicsPipeline>(render_pass->GetServices()->GetAllocator(),
				render_pass,
				usage,
				layout,
				shaders,
				...
				topology,
				tessellation_patch_control_point_count,
				viewports,
				scissors,
				rasterization,
				multisample,
