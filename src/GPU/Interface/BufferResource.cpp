...
namespace np::gpu
{
	mem::sptr<BufferResource> BufferResource::Create(mem::sptr<Device> device, BufferResourceUsage usage, siz size,
													 const con::vector<DeviceQueueFamily>& queue_families)
	{
		...
		case DetailType::Vulkan:
			resource = mem::create_sptr<__detail::VulkanBufferResource>(device->GetServices()->GetAllocator(), device, usage,
																		size, queue_families);
			break;
		}
