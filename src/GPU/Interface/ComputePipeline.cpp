...
namespace np::gpu
{
	mem::sptr<ComputePipeline> ComputePipeline::Create(mem::sptr<Shader> shader, mem::sptr<PipelineResourceLayout> layout, PipelineUsage usage, mem::sptr<PipelineCache> cache)
	{
		...
		case DetailType::Vulkan:
			pipeline = mem::create_sptr<__detail::VulkanComputePipeline>(shader->GetServices()->GetAllocator(), shader, layout, usage, cache);
			break;
