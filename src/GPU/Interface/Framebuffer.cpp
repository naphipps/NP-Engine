...
namespace np::gpu
{
	mem::sptr<Framebuffer> Framebuffer::Create(mem::sptr<RenderPass> render_pass, siz width, siz height, siz layer_count,
											   const con::vector<mem::sptr<ImageResourceView>>& views)
	{
		...
		case DetailType::Vulkan:
			framebuffer = mem::create_sptr<__detail::VulkanFramebuffer>(render_pass->GetServices()->GetAllocator(), render_pass,
																		width, height, layer_count, views);
			break;
