...
namespace np
{
	...
	void ContactListener::BeginContact(b2ParticleSystem* particleSystem, b2ParticleBodyContact* particleBodyContact)
	{
		...
		if (this->postFixtureParticleCollisionBeginBodies.find(b2body) != this->postFixtureParticleCollisionBeginBodies.end())
		{
	...
	void ContactListener::EndContact(b2Fixture* fixture, b2ParticleSystem* particleSystem, int32 particleIndex)
	{
		...
		if (this->duringFixtureParticleCollisionEndBodies.find(b2body) != this->duringFixtureParticleCollisionEndBodies.end())
		{
			// b2body has collision events
			((Body*)b2body->GetUserData())->DuringEndContact(fixture, particleSystem, particleIndex);
		}

		if (this->postFixtureParticleCollisionEndBodies.find(b2body) != this->postFixtureParticleCollisionEndBodies.end())
		{
			// b2body has post collision events
			PostFixtureParticleCollisionParameters postParameters;
			postParameters.body = (Body*)b2body->GetUserData();
	...
	{
		if (this->duringParticleCollisionBeginSystems.find(particleSystem) != this->duringParticleCollisionBeginSystems.end())
		{
			// particle system has collision events
			((ParticleSystem*)particleSystem->GetUserData())->DuringBeginContact(particleSystem, particleContact);
		}

		if (this->postParticleCollisionBeginSystems.find(particleSystem) != this->postParticleCollisionBeginSystems.end())
		{
			// particle system has post collision events
	...
	{
		if (this->duringParticleCollisionEndSystems.find(particleSystem) != this->duringParticleCollisionEndSystems.end())
		{
			// particle system has collision events
			((ParticleSystem*)particleSystem->GetUserData())
				->DuringEndContact(particleSystem, firstParticleIndex, secondParticleIndex);
		}

		if (this->postParticleCollisionEndSystems.find(particleSystem) != this->postParticleCollisionEndSystems.end())
		{
			// particle system has post collision events
			PostParticleCollisionParameters postParameters;
			postParameters.particleSystem = (ParticleSystem*)particleSystem->GetUserData();
	...
	void ContactListener::TriggerPostStepCollisionEvents()
	{
		...
		for (std::vector<PostFixtureCollisionParameters>::iterator iterator = this->postFixtureCollisionBeginEvents.begin();
		...
		for (std::vector<PostFixtureCollisionParameters>::iterator iterator = this->postFixtureCollisionEndEvents.begin();
		...
		for (std::vector<PostParticleCollisionParameters>::iterator iterator = this->postParticleCollisionBeginEvents.begin();
		...
		for (std::vector<PostParticleCollisionParameters>::iterator iterator = this->postParticleCollisionEndEvents.begin();
